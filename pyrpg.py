...
class PyRPG:
    def __init__(self):
        ...
        # パーティの作成
        self.party = Party()
        player1 = Player("swordman_female", (16,27), DOWN, True, self.party)
        player2 = Player("elf_female2", (16,27), DOWN, False, self.party)
        player3 = Player("priestess", (16,27), DOWN, False, self.party)
        player4 = Player("magician_female", (16,27), DOWN, False, self.party)
        ...
        # コマンドウィンドウ
        self.cmdwnd = CommandWindow(Rect(16,16,128,160), self.msg_engine)
        # アイテムウィンドウ
        self.itemwnd = ItemWindow(Rect(16,16,608,448), self.msg_engine)
        # タイトル画面
    ...
    def cmd_handler(self, event):
        ...
        # 矢印キーでコマンド選択
        if event.type == KEYUP and event.key == K_UP:
            if self.cmdwnd.command == 0 : return
        ...
        if event.type == KEYDOWN and event.key == K_SPACE:
            if self.cmdwnd.command == CommandWindow.TALK:  # はなす
                ...
                if treasure != None:
                    ...
                    Data.itemlist.append(treasure.item)
                    print(Data.itemlist)
                    self.game_state = TALK
...
class CommandWindow(Window):
    LINE_HEIGHT = 8  # 行間の大きさ
    TALK, SEARCH, ITEM, DOOR = range(0, 4)
    COMMAND = ["はなす", "しらべる", "どうぐ", "とびら"]
    def __init__(self, rect, msg_engine):
        Window.__init__(self, rect)
...
class ItemWindow(Window):
    MAX_CHARS_PER_LINE = 24    # 1行の最大文字数
    MAX_LINES_PER_PAGE = 18
    MAX_CHARS_PER_PAGE = 24*18
    LINE_HEIGHT = 8  # 行間の大きさ
    def __init__(self, rect, msg_engine):
        Window.__init__(self, rect)
        self.text_rect = self.inner_rect.inflate(-32, -32)
    ...
    def draw(self, screen):
        Window.draw(self, screen)
        if self.is_visible == False: return
        k = Data.itemlist # .sort()
        num = len(k)

        for i in range(0, num):
            if i >= 39:
                dx = self.text_rect[0] + MessageEngine.FONT_WIDTH * 27
                dy = self.text_rect[1] + (self.LINE_HEIGHT+MessageEngine.FONT_HEIGHT) * (i - 39)
            elif i>=26:
                dx = self.text_rect[0] + MessageEngine.FONT_WIDTH * 18
                dy = self.text_rect[1] + (self.LINE_HEIGHT+MessageEngine.FONT_HEIGHT) * (i - 26)
            elif i >= 13:
                dx = self.text_rect[0] + MessageEngine.FONT_WIDTH * 9
                dy = self.text_rect[1] + (self.LINE_HEIGHT+MessageEngine.FONT_HEIGHT) * (i - 13)
            else:
                dx = self.text_rect[0] + MessageEngine.FONT_WIDTH
                dy = self.text_rect[1] + (self.LINE_HEIGHT+MessageEngine.FONT_HEIGHT) * i
            self.msg_engine.draw_string(screen, (dx,dy), str(k[i]))
        # for i in range(4, 8):
        #     dx = self.text_rect[0] + MessageEngine.FONT_WIDTH * 8
